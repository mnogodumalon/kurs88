...
async def main():
    ...
    async def create_apps(args):
        ...
        print(f"{'='*80}")
        print(response_payload[:3000])
        if len(response_payload) > 3000:
            print(f"  ... ({len(response_payload) - 3000} more chars truncated) ...")
        print(f"{'='*80}\n")
    ...
    async def generate_typescript(args):
        ...
        try:
            ...
            if len(response_text) > 5000:
                print(f"\n  ... ({len(response_text) - 5000} more chars truncated) ...")
            print(f"{'='*80}\n")
...
Starte JETZT mit Schritt 1!"""
    ...
    if claude_md_path.exists():
        ...
        if len(claude_md) > 2000:
            print(f"  ... ({len(claude_md) - 2000} more chars truncated) ...")
        print(f"{'='*80}")
    else:
    ...
    if skills_dir.exists():
        for skill_file in skills_dir.rglob("*.md"):
            ...
            if len(skill_content) > 500:
                print(f"  ... ({len(skill_content) - 500} more chars) ...")
    else:
